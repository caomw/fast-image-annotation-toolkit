$ ->
    ...
    class Shape
        constructor: (@canvas, @x, @y, @w, @h, @fill = '#AAA') ->
            @selectionHandles = []
    ...
    class Box
        constructor: (@canvas, @pos, @color='darkred', @x=0, @y=0, @w=10, @h=10) ->
        ...
        draw: (canvas = @canvas) ->
            if @x > canvas.width or @y > canvas.height or
               @x + @w < 0 or @y + @h < 0
                return @
    ...
    class Canvas
        constructor: (@canvas) ->
            ...
            @canvas.dblclick (e) ->
                mouse = canvas.getMouse e
                canvas.addShape new Shape canvas, mouse.x-10, mouse.y-10, 20, 20,
                               'rgba(0, 255, 0, .6)'
        ...
        draw: ->
            if not @isValid
                ...
                for shape in @shapes
                    if shape.x > @width or shape.y > @height or
                       shape.x + shape.w < 0 or shape.y + shape.h < 0
                        continue
                    shape.draw()
    ...
    init = ->
        canvas = $('#my-canvas')
        s = new Canvas(canvas)
        s.addShape new Shape s, 40, 40, 50, 50
        s.addShape new Shape s, 60, 140, 40, 60, 'lightskyblue'
        s.addShape new Shape s, 80, 150, 60, 30, 'rgba(127, 255, 212, .5)'
        s.addShape new Shape s, 125, 80, 30, 80, 'rgba(245, 222, 179, .7)'
    init()
    @
